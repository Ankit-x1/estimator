...
from estedge.backend.base import Backend
from estedge.backend.numpy_backend import NumPyBackend
from estedge.core.covariance import update_covariance
from estedge.core.state import State
from estedge.core.time import clamp_dt
...
from estedge.noise.static import StaticNoise
from estedge.utils.checks import check_measurement, check_state
from estedge.utils.linalg import enforce_psd, symmetrize
...
class KF:
    ...
    def update(self, z: np.ndarray, timestamp: Optional[float] = None) -> State:
        ...
        z = np.asarray(z)
        if z.ndim != 1:
            raise ValueError(f"Measurement must be 1D, got shape {z.shape}")
        if z.shape[0] != self.measurement_model.measurement_dim:
            raise ValueError(
                f"Measurement dimension mismatch: expected {self.measurement_model.measurement_dim}, "
                f"got {z.shape[0]}"
            )
...
class EKF:
    ...
    def __init__(
        ...
        if noise_model is None:
            ...
            # For multi-sensor, we can't use a single R
            # StaticNoise will use the first R, but adaptive_noise should handle per-sensor R
        ...
        # Initialize adaptive noise with per-sensor R if provided
        if (
            self.adaptive_noise
            and hasattr(self.adaptive_noise, "single_sensor")
            and self.adaptive_noise.single_sensor
        ):
            # Convert single-sensor adaptive noise to multi-sensor if needed
            if len(measurement_models) > 1:
                R_init_dict = {
                    name: model.covariance() for name, model in measurement_models.items()
                }
                # Reinitialize adaptive noise with per-sensor R
                from estedge.noise.adaptive import AdaptiveNoise

                self.adaptive_noise = AdaptiveNoise(
                    Q_init=self.adaptive_noise.Q,
                    R_init_dict=R_init_dict,
                    window_size=self.adaptive_noise.window_size,
                    adaptation_rate=self.adaptive_noise.adaptation_rate,
                )
    ...
    def update(self, z: np.ndarray, sensor_name: str, timestamp: Optional[float] = None) -> State:
        """
        ...
        if z.ndim != 1:
            raise ValueError(f"Measurement must be 1D, got shape {z.shape}")
        if z.shape[0] != measurement_model.measurement_dim:
            raise ValueError(
                f"Measurement dimension mismatch: expected {measurement_model.measurement_dim}, "
                f"got {z.shape[0]} for sensor {sensor_name}"
            )
        ...
        R = measurement_model.covariance()
        if self.adaptive_noise:
            R = self.adaptive_noise.get_R(sensor_name=sensor_name)
        ...
        if self.adaptive_noise:
            self.adaptive_noise.add_innovation(innovation, S, sensor_name=sensor_name)
            self.adaptive_noise.adapt_R(sensor_name=sensor_name)
...
class UKF:
    ...
    def update(self, z: np.ndarray, sensor_name: str, timestamp: Optional[float] = None) -> State:
        ...
        # Covariance update: use Joseph form for numerical stability
        # For UKF, compute effective H from cross-covariance: H_eff = Pxz^T @ P^-1
        # But simpler: use standard UKF update with PSD enforcement
        # P = P - K @ S @ K.T (standard UKF)
        # For numerical stability, we enforce symmetry and PSD
